...
const InfoItem: React.FC<{ label: string; value: string | number; isCurrency?: boolean; isPercent?: boolean; valueClass?: string; }> = ({ label, value, isCurrency = false, isPercent = false, valueClass = 'text-gray-800' }) => (
  <div className="flex justify-between items-center py-2.5 border-b border-gray-200/80 text-sm">
...
const FinancialTable: React.FC<{ title: string; children: React.ReactNode; }> = ({ title, children }) => (
    <div className="mb-6">
        <h4 className="text-sm font-semibold text-gray-500 bg-gray-100/80 px-3 py-2 rounded-t-md border-b-2 border-gray-200">{title}</h4>
        <div className="px-3">
          {children}
        </div>
    </div>
...
const BusinessPlan = forwardRef<HTMLDivElement, BusinessPlanProps>(
    ...
    const productsOnPage1 = data.products.slice(0, 2);
    const productsOnPage2 = data.products.slice(2);
        ...
        return (
            <FinancialTable key={product.id} title={`Product: ${product.nexstarModel}`}>
                ...
                <div className="mt-4 pt-4 border-t border-dashed border-gray-300">
                    <h5 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Unit Economics</h5>
                ...
                <main>
                    <section className="mb-10">
                        ...
                        <div className="bg-gray-50 border border-gray-200 p-5 rounded-lg">
                            <p className="text-gray-600 leading-relaxed text-sm">{data.aiSummary || "No summary available."}</p>
                        </div>
                    </section>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-8 mb-12">
                        <div>
                            ...
                            <div className="space-y-4">
                                <OverviewMetricCard title="Total Sales" value={`$${data.totalSales.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`} colorClass="bg-green-50" />
                                <OverviewMetricCard title="Net Profit" value={`$${data.netProfit.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`} colorClass="bg-blue-50" />
