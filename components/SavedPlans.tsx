...
const SavedPlans: React.FC<SavedPlansProps> = ({ plans, history, onSelectPlan, onDeletePlan, onDuplicatePlan, onEditPlan, onNewPlan, onViewHistoryItem, onUpdateHistoryStatus }) => {
    return (
        <div className="bg-surface p-6 sm:p-8 rounded-xl shadow-lg border border-gray-200 animate-fade-in">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 border-b border-gray-200 pb-4">
                <h2 className="text-3xl font-bold text-text-primary mb-2 sm:mb-0">Saved Business Plans</h2>
                <button onClick={onNewPlan} className="bg-accent text-white font-bold py-2 px-5 rounded-lg hover:bg-accent-hover transition-all duration-300 flex items-center w-full sm:w-auto justify-center shadow-lg shadow-accent/20 hover:shadow-xl hover:shadow-accent/30 transform hover:-translate-y-0.5">
                    <PlusIcon /> New Plan
                </button>
            </div>
            ...
            {plans.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {plans.map((plan, index) => (
                        <div key={plan.id} 
                             className="border border-gray-200 rounded-lg shadow-md flex flex-col justify-between bg-surface hover:-translate-y-1 hover:shadow-xl hover:border-primary/50 transition-all duration-300 group"
                            ...
                            <div className="flex items-center justify-between p-3 bg-background border-t border-gray-200 rounded-b-lg">
                                <div className="flex space-x-1">
                                    <button onClick={(e) => { e.stopPropagation(); onDeletePlan(plan.id); }} className="text-sm text-danger/70 hover:text-danger p-2 rounded-md hover:bg-danger/10" aria-label={`Delete plan ${plan.planName}`} title="Delete">
                                        <TrashIcon />
                                    </button>
